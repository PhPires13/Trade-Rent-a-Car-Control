...
class Cobranca(models.Model):
    ...
    def atualizar_status(self, hoje=None):
        ...
        if self.status in (self.Status.JUDICIAL, self.Status.CANCELADA):
            return
        if self.saldo <= ZERO:
...
class Caucao(models.Model):
    ...
    def _soma(self, *tipos):
        return self.movimentacoes.filter(tipo__in=tipos).aggregate(t=Sum("valor"))["t"] or ZERO

    @property
    def recebido(self):
        return self._soma("recebimento", "reforco")

    @property
    def descontado(self):
        return self._soma("desconto")

    @property
    def devolvido(self):
        return self._soma("devolucao")
